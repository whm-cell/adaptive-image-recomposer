...
The compiler turns verified content, content-group bindings, one selected structural kernel, and one paired visual system into a renderer-specific work contract. The catalog may be large; the final prompt must remain narrow.
...
The kernel-specific hints supply the actual topology, not this generic sentence.
...
Edit the latest result and name one defect. Repeat:
...
- Label every input as edit target, factual content reference, style reference, or insert asset.
- Do not promote unverified OCR into required copy.
